...
def create_sect(image, width_start_end, length, incs, conversion_type, colors, filename):
    w_start, w_end = width_start_end[0], width_start_end[1]
    l_inc, w_inc = incs[0], incs[1]

    legofied_sect = [[None for x in range(length)] for y in range(w_end - w_start)]

    for r in range(w_start, w_end):
        for c in range(length):
            ...
            if conversion_type == 3:
                ...
                legofied_sect[r-w_start][c] = lego_hex
            else:
                if conversion_type == 1:
                    current_rgb = single_pix(copy)
                elif conversion_type == 2:
                    current_rgb = dominant_color_post(copy)
                legofied_sect[r-w_start][c] = closest_color(current_rgb, colors)

    save_to_file(legofied_sect, filename, colors==LEGO)
...
def create_legofied(length, image_name, conversion_type, colors):
    ...
    a, b, c, d = div_four(width)  # used to divide the image into 4 (almost) equal parts

    l_inc = col // length
    w_inc = row // width
    incs = (l_inc, w_inc)

    process_a = Process(target=create_sect, args=(image, (0, a), length, incs, conversion_type, colors, 'temp_a.txt', ))
    process_b = Process(target=create_sect, args=(image, (a, a+b), length, incs, conversion_type, colors, 'temp_b.txt', ))
    process_c = Process(target=create_sect, args=(image, (a+b, a+b+c), length, incs, conversion_type, colors, 'temp_c.txt', ))
    process_d = Process(target=create_sect, args=(image, (a+b+c, a+b+c+d), length, incs, conversion_type, colors, 'temp_d.txt', ))
    ...
    sect_a = load_from_file('temp_a.txt')[0]
    sect_b = load_from_file('temp_b.txt')[0]
    sect_c = load_from_file('temp_c.txt')[0]
    sect_d = load_from_file('temp_d.txt')[0]
    legofied = sect_a + sect_b + sect_c + sect_d
...
def save_to_file(legofied, filename, use_lego):
